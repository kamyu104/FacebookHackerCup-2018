    ...
    for i in xrange(N):
        ...
        if not max_D:
            ...
            while max_D and D[max_D[-1]] <= D[i]:  # keep descending
                r = max_D.pop()
                segment_tree.update(r, r, float("inf"))  # remove impossible candidate costs
            if max_D:
                segment_tree.update(i, i, dp[max_D[-1]+1] + S + D[i])  # dig D[i] with dp[max_D[-1]+1]
            max_D.append(i)
            dp[i+1] = min(dp[(j-1)+1] + S + D[max_D[0]],      # min(dig max{D[j..i]} with dp[(j-1)+1],
